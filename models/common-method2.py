...
class SubConv2d(nn.Conv2d):
    ...
    def forward(self, x): 
        ...
        if self.BrokenTarget is not None:
            return F.conv2d(x, self.weight, stride=self.stride, padding=self.padding, groups=self.groups)
        else:
            ...
            print('self.weight.permute(2, 3, 0, 1).reshape(-1, self.in_channels, 1, 1)', self.weight.permute(2, 3, 0, 1).reshape(-1, self.in_channels, 1, 1).shape)
            print('self.FilterSkeleton.permute(1, 2, 0).reshape(-1)', self.FilterSkeleton.permute(1, 2, 0).reshape(-1).unsqueeze(1).shape)
            x = F.conv2d(x, self.weight.permute(2, 3, 0, 1).reshape(-1, self.in_channels, 1, 1) * self.FilterSkeleton.reshape((-1,1,1)).unsqueeze(1), stride=self.stride, padding=self.padding, groups=self.groups)
            l, h = 0, 0
            for i in range(self.FilterSkeleton.detach().sum(dim=0).shape[0]):
                for j in range(self.FilterSkeleton.detach().sum(dim=0).shape[1]):
                    h += self.FilterSkeleton.detach()[:, i, j].sum().item()
                    # print('i:', type(i), 'j', type(j))
                    # print('l', type(l))
                    # print('h', type(h))
                    # print('x', type(x))
                    #print('===', self.shift_c(x[:, l:h], i, j))
                    print(self.shift_c(x[:, l:h], i, j))
                    #out[:, self.FilterSkeleton.detach()[:, i, j]] += self.shift_c(x[:, l:h], i, j)[:, :, ::self.stride[0], ::self.stride[1]]
                    out[:, self.FilterSkeleton.detach()[:, i, j]] += self.shift_c(x[:, l:h], i, j)[:, :, ::self.stride[0], ::self.stride[1]]
                    l += self.FilterSkeleton.detach()[:, i, j].sum().item()
            return out
    ...
    def _break(self, threshold):
        ...
        # 对三维的FS, 则是在第一个维度进行合并，就是说，FS所有的channel合并成一个channel
    ...
    def shift_c(self, x, i, j):
        print('======================')
        # print('self.FilterSkeleton.detach().shape[0]', self.FilterSkeleton.detach().shape[0] // 2 - j)
        # print('j - self.FilterSkeleton.detach().shape[0] // 2', j - self.FilterSkeleton.detach().shape[0] // 2)
        # print('self.FilterSkeleton.detach().shape[0] // 2 - i', self.FilterSkeleton.detach().shape[0] // 2 - i)
        # print('i - self.FilterSkeleton.detach().shape[1] // 2)', i - self.FilterSkeleton.detach().shape[1] // 2)
        self.FilterSkeleton = self.FilterSkeleton.detach().abs() > threshold
        return F.pad(x, (self.FilterSkeleton.shape[0] // 2 - j, j - self.FilterSkeleton.shape[0] // 2, self.FilterSkeleton.shape[0] // 2 - i, i - self.FilterSkeleton.shape[1] // 2), 'constant', 0)
