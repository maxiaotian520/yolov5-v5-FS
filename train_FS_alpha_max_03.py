...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if epoch == 299 and i == 1848: # 当batch-size =112, 共有36个i, 从0开始，最后一个为35   (eg. i=58 进度条一共259个batch)    299 and i == 1848
                # 获取FS 在优化器中每个值的坐标
                alpha_list_orig = optimizer.step_alpha_getList()
                #print('alpha_list_orig:', len(alpha_list_orig))
                ...
                while True: 
                    try:
                        ...
                        # 思路： 给loss_new 换个芯儿，保留其他的属性和梯度，只单纯更换数字。它里面的梯度不是我们说的参数的梯度
                        ...
                        if index > 0:
                            ...
                            #optimizer.step(settings=al, parameters=params_new, parameters_orig=param_groups, settings_last=al_last)
                            optimizer.step(settings=al, parameters_orig=param_groups, settings_last=al_last, index=index)
                            
                        else: # index == 0
                            # 导出原始的params, 使得每次param更新不是真的更新，而是用这个模板不断恢复，只删除坐标对应的那一个
                            # 这里param_groups 不是最后要set 0 的那波参数，它进入optimizer后，替换了上一波被更新过的参数
                            _, param_groups = optimizer.step(settings=al, index=index)
                            

                        optimizer.zero_grad()

                        # # 用来测试optimizer.step 里值的改变是否在model里也改变，并且借助al_old测试是否每次只删一个数，之前的数是否被重新用parameter覆盖
                        # if al_last is not None and index > 1:
                        #     g4 = []
                        #     for v in model.modules():
                        #         #悬挂需要更新的Conv_pt 中的FS， 不包括weight
                        #         if hasattr(v, 'FilterSkeleton'):
                        #             g4.append(v.FilterSkeleton)
                        #     #这里无法调用.grad查看梯度，因为pytorch不允许
                        #     print('g4[al[0]][al[1]][al[2]][al[3]]---', g4[al[0]][al[1]][al[2]][al[3]])
                        #     print('g4[al_old[0]][al_old[1]][al_old[2]][al_old[3]]===', g4[al_last[0]][al_last[1]][al_last[2]][al_last[3]])
